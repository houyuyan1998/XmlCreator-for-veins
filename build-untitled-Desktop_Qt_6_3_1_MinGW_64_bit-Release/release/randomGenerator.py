...
def filter(route,junction_edge):
    i=0
    junction=[]
...
address2:文件储存地址
'''
def library(step,n,address1,address2):
    waytype={}
    ...
    fr=open(address1,"r",encoding="UTF-8")
    edgetype={}
    line=fr.readline()
    while line:
        line=fr.readline()
        if re.match("    <edge priority=\"\S*\" from",line):
            ID=re.findall(".*id=\"(\S*)\"",line)
            ID=ID[0]
            #print(ID)
    ...
    fw=open(address2,"w",encoding="UTF-8")
    ...
    fw1=open(address2[:-7]+"circle library.txt","w",encoding="UTF-8")
    fw0=open(address2[:-7]+"noncircle library.txt","w",encoding="UTF-8")
    circle=0
    line=fr.readline()
    while line:
        route=[]
        ...
        if re.match("		<route edges=",line):
            ...
            route=route[0].split()
            #print(route)
            circle=filter(route,junction_edge)
            i=0
            #flag=0
            #junction=[]
            #junction.append(junction_edge[route[i]][0])
...
#address1="D:\\test3\\osm.net.xml"
#address2="D:\\test3\\abc.rou.xml"
#library(20,1000,address1,address2)
